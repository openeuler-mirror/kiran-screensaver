...
bool KSFadeXrandr::checkForSupport()
{
    bool support = false;
...
bool KSFadeXrandr::setAlphaGamma(double alpha)
{
...
void KSFadeXrandr::finish()
{
    cleanup();
}

void KSFadeXrandr::cleanup()
{
    for(auto iter=d_ptr->crtcGammInfo.begin();
         iter!=d_ptr->crtcGammInfo.end();
         )
